...
### Database Design
The database schema supports the group-based permission system with relationships between users, groups, and permissions.

### Backend Permission Checks

#### Implementation Pattern
Backend permission checks are performed using the user's permission set, which is resolved from all groups the user belongs to.

#### Example: Deployment History Permission Check

```python
# Example: Deployment history permission check
permissions = request.zeuz_user.permissions  # single call

if not request.zeuz_user.has_permission("testing.deployments.history.view"):
    return JsonResponse({
        "total": [],
        "no_permission": True,
        "message": "You don't have permission to access deployment history data"
    })
```

#### Example: Field-Level Permission Filtering

```python
# Example: Field-level permission filtering
if "testing.deployments.history.run_id.view" not in permissions:
    filtered_row[0] = "No Permission"  # Run ID
```

### Best Practices
- Always check permissions before returning sensitive data.
- Use field-level checks for granular access control.
- Return clear error messages when permissions are denied.

### Configuration Structure

The `permissions.json` file follows a hierarchical structure organized by feature areas:
    ...
    "deployments": {
      ...
      "permissions": {
        "run_tests": {
          "name": "Run Tests",
          "permissions": [
            { "id": "testing.deployments.run_tests.view", "name": "View" }
          ]
        }
        // ... more permissions
      }
...
1. **Add to `permissions.json`**: Define the permission in the appropriate section of the JSON file.
2. **Backend Implementation**: Implement the corresponding permission check in the backend code.
3. **Frontend Integration**: The permission will automatically appear in the Group management UI.

### Permission ID Format

Permission IDs follow a dot-separated hierarchical naming convention:

```
<category>.<feature>.<subfeature>.<action>
```

**Example**: `testing.deployments.history.run_id.view`

- `testing` - Category
- `deployments` - Feature
- `history` - Subfeature
- `run_id` - Specific element
- `view` - Action

### Summary
- **Centralized Control**: Permissions are managed at the Group level, not per project.
- **Additive Logic**: Users inherit permissions from all groups they belong to (OR logic).
- **Global Scope**: Permissions apply across all Team Projects.
- **System Admin**: Highest privilege level with exclusive capabilities.
- **Configuration**: Frontend permissions defined in `permissions.json`.
- **Backend Checks**: Implement permission validation using `request.zeuz_user.has_permission()`
